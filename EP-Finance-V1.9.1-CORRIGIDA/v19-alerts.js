...
(function(){
  ...
  const INSTALL_DISMISSED_KEY='epFinancePwaInstallDismissed';
  const INSTALL_DISMISSED_AT_KEY='epFinancePwaInstallDismissedAt';

  function biometricLockVisible(){
    const lock=$id('biometricLock');
  ...
  function maybeShowInstall(){
    ...
    const shouldShow=iosSafari() && !standalone() && !dismissed && !biometricLockVisible();
    if(shouldShow) showInstallBanner(); else hideInstallBanner();
  }
  ...
  function bind(){
    ...
    $id('dismissPwaInstall')?.addEventListener('click',dismissInstallBanner);
    $id('pwaInstallHow')?.addEventListener('click',installHow);
    const lock=$id('biometricLock');
    if(lock){new MutationObserver(()=>maybeShowInstall()).observe(lock,{attributes:true,attributeFilter:['hidden']});}
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){renderAlertCenter();maybeShowInstall();}});
