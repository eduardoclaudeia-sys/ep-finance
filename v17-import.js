...
(function(global){
  ...
  const num=s=>{ const v=norm(s).replace(/\./g,'').replace(',','.').replace(/[^0-9.-]/g,''); const n=Number(v); return Number.isFinite(n)?n:0; };
  const cleanDate=s=>{ const x=norm(s).replace(/[^0-9]/g,''); if(x.length>=8) return `${x.slice(0,4)}-${x.slice(4,6)}-${x.slice(6,8)}`; return norm(s).slice(0,10); };
